...
@section('content')
    ...
    <form action="{{route('shopify.products')}}">
        <div class="row">
            <div class="col-md-5">
                        ...
                        <table class="table v-middle" id="order_table">
                            ...
                            @foreach($products as $key=>$product)
                                <tr @if($product->has_print_product!==null) class="bg-light" @endif>
                                    ...
                                    <td><a href="{{route('shopify.product.detail',encrypt($product->id))}}"><img src="{{$product->image}}" width="70px" height="auto"
                                                class="img-thumbnail"/></a></td>
                                    <td><a href="{{route('shopify.product.detail',encrypt($product->id))}}">{{$product->title}}</a></td>
